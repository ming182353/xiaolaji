<template>
    <div>
        <a href="javascript:;" class="select-city" @click="showSelectCity">
            ...
            <span>{{city}}<i class="down-arrow"></i></span>
        </a>
            ...
            <div class="select-city-group">
                <div>
                    ...
                    >
                        ...
                        <ul class="list">
                            <li 
                                class="item" v-for="(item2,index) in item"
                                :key="index"
                                @click="selectCity(item2)"
                            >{{item2.split('|')[0]}}</li>
                        </ul>
                    </div>
                </div>
...
let allCities = ['北京|beijing|bj', '上海|shanghai|sh', '重庆|chongqing|cq', '深圳|shenzhen|sz', '广州|guangzhou|gz', '杭州|hangzhou|hz',
...
export default {
  ...
  methods: {
    ...
    selectCity (item) {
      ...
      //1,v-model形式
      this.$emit('input', item.split('|'))
      //sync 模式
      //2,this.$emit('update:cityId', item.split('|'))
    }
  },
  created () {
    this.setAllCity()
...
.select-city{
  ...
  span{
      ...
      .down-arrow{
          width: 10px;
          height: 10px;
          border-left: 1px solid #928ea5;
          border-bottom: 1px solid #928ea5;
